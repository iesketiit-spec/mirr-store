...
// IMPORTANTE: Mueve este valor a .env.local como NEXT_PUBLIC_PAYPAL_CLIENT_ID
// y NO subas ese archivo a GitHub
const PAYPAL_CLIENT_ID = process.env.NEXT_PUBLIC_PAYPAL_CLIENT_ID || "BAASrpcwwEA6WbQyfG-EAnzsS21sRhF-gom0gQb"
                ...
                {checkoutSteps.map((step, index) => (
                  <div key={step.id} className="flex items-center gap-3">
                    ...
                    {index < checkoutSteps.length - 1 && (
                      <div className={`w-12 h-px ${currentStep > step.id ? "bg-foreground" : "bg-border"}`} />
                    )}
                  </div>
                ))}
              </nav>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        <div>
                          <label className="block text-xs text-muted-foreground mb-2">Correo electronico *</label>
                          <input
                            type="email" name="email" required value={formData.email}
                            onChange={handleInputChange} placeholder="ejemplo@correo.com"
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={handleInputChange} placeholder="+57 320 123 4567"
                            className="w-full px-4 py-3 bg-input border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent text-base"
                          />
                        </div>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        <div>
                          <label className="block text-xs text-muted-foreground mb-2">Nombre completo *</label>
                          <input
                            type="text" name="name" required value={formData.name}
                            onChange={handleInputChange} placeholder="Juan Perez"
                            className="w-full px-4 py-3 bg-input border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent text-base"
                          />
                        </div>
                        <div>
                          <label className="block text-xs text-muted-foreground mb-2">Direccion *</label>
                          <input
                            type="text" name="address" required value={formData.address}
                            onChange={handleInputChange} placeholder="Calle 123 #45-67"
                            className="w-full px-4 py-3 bg-input border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent text-base"
                          />
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            <label className="block text-xs text-muted-foreground mb-2">Pais *</label>
                            <select
                          ...
                          <div>
                            <label className="block text-xs text-muted-foreground mb-2">Ciudad *</label>
                            <input
                              type="text" name="city" required value={formData.city}
                              onChange={handleInputChange} placeholder="Medellin"
                              className="w-full px-4 py-3 bg-input border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent text-base"
                            />
                          </div>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            <label className="block text-xs text-muted-foreground mb-2">Departamento / Provincia *</label>
                            <input
                              type="text" name="state" required value={formData.state}
                              onChange={handleInputChange} placeholder="Antioquia"
                              className="w-full px-4 py-3 bg-input border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent text-base"
                            />
                          </div>
                          <div>
                            <label className="block text-xs text-muted-foreground mb-2">Codigo postal</label>
                            <input
                              type="text" name="zip" value={formData.zip}
                              onChange={handleInputChange} placeholder="050001"
                              className="w-full px-4 py-3 bg-input border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent text-base"
                            />
                          </div>
                        </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-3 mb-6">
                        {paymentMethods.map((method) => (
                          <button
                            key={method.id}
                            type="button"
                            onClick={() => { setPaymentMethod(method.id); setPaypalError("") }}
                      ...
                      {paymentMethod === "card" && (
                        <div className="space-y-4 p-4 border border-border bg-card/30">
                          <div>
                            <label className="block text-xs text-muted-foreground mb-2">Numero de tarjeta *</label>
                            <input
                              type="text" name="cardNumber" required value={formData.cardNumber}
                              onChange={handleInputChange} placeholder="1234 5678 9012 3456"
                              className="w-full px-4 py-3 bg-input border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent text-base"
                            />
                          </div>
                          <div>
                            <label className="block text-xs text-muted-foreground mb-2">Nombre en la tarjeta *</label>
                            <input
                              type="text" name="cardName" required value={formData.cardName}
                              onChange={handleInputChange} placeholder="Juan Perez"
                              className="w-full px-4 py-3 bg-input border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent text-base"
                            />
                          </div>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <label className="block text-xs text-muted-foreground mb-2">Vencimiento *</label>
                              <input
                                type="text" name="cardExpiry" required value={formData.cardExpiry}
                                onChange={handleInputChange} placeholder="MM / AA"
                                className="w-full px-4 py-3 bg-input border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent text-base"
                              />
                            </div>
                            <div>
                              ...
                              <input
                                type="text" name="cardCvv" required value={formData.cardCvv}
                      ...
                      {paymentMethod === "paypal" && (
                        <div className="p-4 border border-border bg-card/30">
                          <p className="text-xs text-muted-foreground mb-4 text-center">
                            Al hacer clic seras redirigido a PayPal para completar tu pago de forma segura.
                          </p>
                          {paypalError && (
                            <p className="text-xs text-red-500 mb-3 text-center">{paypalError}</p>
                          )}
                          <PayPalButtons
                            ...
                            onError={(err) => {
                              ...
                              setPaypalError("Hubo un error con PayPal. Por favor intenta de nuevo.")
                            }}
                            onCancel={() => {
                      ...
                      {paymentMethod === "transfer" && (
                        <div className="p-4 border border-border bg-card/30 space-y-2">
                          <p className="text-sm text-foreground font-semibold">Datos para transferencia:</p>
                          <p className="text-xs text-muted-foreground">Banco: Bancolombia</p>
                          <p className="text-xs text-muted-foreground">Tipo de cuenta: Ahorros</p>
                          <p className="text-xs text-muted-foreground">Numero: 000-000000-00</p>
                          <p className="text-xs text-muted-foreground">A nombre de: MIRR Store</p>
                          <p className="text-xs text-muted-foreground mt-2">
                            Envia el comprobante a <span className="text-accent">pagos@mirrstore.com</span>
                          </p>
                        </div>
                      )}
                    </div>

                    {/* Boton pagar — solo si NO es PayPal */}
                    {paymentMethod !== "paypal" && (
