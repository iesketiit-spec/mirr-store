...
const products: Product[] = [
  // GOTHIC COLLECTION
  { id: "1", name: "Hoodie Gothic", price: 50, currency: "USD", image: "/hoodie-gothic-1.jpeg", sizes: ["S","M","L","XL","XXL"], isNew: true, slug: "hoodie-gothic", category: "hoodies", collection: "gothic" },
  { id: "2", name: "Conjunto Urban Gótico", price: 50, currency: "USD", image: "/hoodie-gothic-2.jpeg", sizes: ["S","M","L","XL"], isNew: true, slug: "conjunto-urban-gotico", category: "conjuntos", collection: "gothic" },

  // MEDELLÍN COLLECTION — Camisetas
  { id: "3", name: "Tee Medellín", price: 18, currency: "USD", image: "/tee-medellin.jpeg", sizes: ["S","M","L","XL"], isNew: true, slug: "tee-medellin", category: "camisetas", collection: "medellin" },
  ...
  // CONJUNTOS & PANTS
  { id: "11", name: "Conjunto Urban Medellín", price: 70, currency: "USD", image: "/conjunto-urban-medellin.jpeg", sizes: ["S","M","L","XL"], isNew: true, slug: "conjunto-urban-medellin", category: "conjuntos", collection: "medellin" },
  { id: "12", name: "Pantalón Sudadera Medellín", price: 35, currency: "USD", image: "/pantalon-medellin.jpeg", sizes: ["S","M","L","XL","XXL"], isNew: true, slug: "pantalon-sudadera-medellin", category: "pantalones", collection: "medellin" },
]
...
const categories = [
  ...
  { id: "conjuntos", name: "Conjuntos", count: products.filter(p => p.category === "conjuntos").length },
  { id: "pantalones", name: "Pantalones", count: products.filter(p => p.category === "pantalones").length },
]

const collections = [
  { id: "all", name: "Todas" },
  { id: "gothic", name: "Gothic Collection" },
  { id: "medellin", name: "Medellín Collection" },
]
        ...
        <section className="relative h-[45vh] min-h-[300px] flex items-end overflow-hidden pt-24">
          ...
          <div className="relative z-10 w-full flex items-end justify-between px-6 lg:px-16 pb-8">
            <div>
              ...
              <p className="text-sm text-foreground/40 tracking-widest mt-1 uppercase">Piezas únicas. Ediciones limitadas. Lleva MIRR contigo.</p>
            </div>
            ...
            {filtersOpen && (
              ...
              >
                <div className="p-6 w-[260px]">
                  ...
                  {/* Categories */}
                  <p className="text-xs tracking-widest uppercase text-foreground/30 mb-3">Categorías</p>
                  ...
                  {/* Collections */}
                  <p className="text-xs tracking-widest uppercase text-foreground/30 mb-3">Colección</p>
                  <div className="space-y-1 mb-6">
                    {collections.map(c => (
                      <button key={c.id} onClick={() => setSelectedCollection(c.id)}
                        className={`w-full text-left text-sm px-2 py-1.5 transition-colors ${selectedCollection === c.id ? "text-foreground font-semibold" : "text-foreground/50 hover:text-foreground"}`}>
                        {c.name}
                      </button>
                    ))}
                  </div>
                    ...
                    <Link href={`/producto/${product.slug}`}>
                      <div className="relative aspect-[3/4] overflow-hidden">
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-background/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3">
                          <button className="w-full bg-white text-black text-[10px] tracking-widest uppercase py-2.5 font-bold hover:bg-red-600 hover:text-white transition-colors">
                            Agregar al carrito
                          </button>
                        </div>
                      </div>
